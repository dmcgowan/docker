...
func (cli *DockerCli) getPassphraseRetriever() passphrase.Retriever {
	...
	rootEnv := os.Getenv("DOCKER_TRUST_ROOT_PASSPHRASE")
	targetEnv := os.Getenv("DOCKER_TRUST_TARGET_PASSPHRASE")
	snapshotEnv := os.Getenv("DOCKER_TRUST_SNAPSHOT_PASSPHRASE")

	return func(keyName string, alias string, createNew bool, numAttempts int) (string, bool, error) {
		if alias == "root" && rootEnv != "" {
			return rootEnv, numAttempts > 1, nil
		}
		if alias == "targets" && targetEnv != "" {
			return targetEnv, numAttempts > 1, nil
		}
		if alias == "snapshot" && snapshotEnv != "" {
