...
func (lm *LayerMigrator) MigrateTocontainerd(ctx context.Context, snKey string, sn snapshots.Snapshotter) error {
	...
	for imgID, img := range lm.dis.Heads() {
		...
		for i := range diffids {
			chainID := layer.CreateChainID(diffids[:i])
		...
		if err = content.WriteBlob(ctx, lm.content, "", bytes.NewReader(configBytes), manifest.Config); err != nil && !errdefs.IsAlreadyExists(err) {
			return err
		}
		...
		if err = content.WriteBlob(ctx, lm.content, "", bytes.NewReader(manifestBytes), manifestDesc); err != nil && !errdefs.IsAlreadyExists(err) {
			return err
		}
