...
func TestSave(t *testing.T) {
	...
	for refStr, id := range saveLoadTestCases {
		...
		if canonical, ok := ref.(reference.Canonical); ok {
			...
			err = store.AddTag(reference.FamiliarName(ref), id, false)
			if err != nil {
...
func TestAddDeleteGet(t *testing.T) {
	...
	ref1, err := reference.NormalizedName("username/repo1:latest")
	if err != nil {
		t.Fatalf("could not parse reference: %v", err)
	}
	...
	ref2, err := reference.NormalizedName("username/repo1:old")
	if err != nil {
		t.Fatalf("could not parse reference: %v", err)
	}
	...
	ref3, err := reference.NormalizedName("username/repo1:alias")
	if err != nil {
		t.Fatalf("could not parse reference: %v", err)
	}
	...
	ref4, err := reference.NormalizedName("username/repo2:latest")
	if err != nil {
		t.Fatalf("could not parse reference: %v", err)
	}
	...
	ref5, err := reference.NormalizedName("username/repo3@sha256:58153dfb11794fad694460162bf0cb0a4fa710cfa3f60979c177d920813e267c")
	if err != nil {
		t.Fatalf("could not parse reference: %v", err)
	}
	...
	nonExistRepo, err := reference.NormalizedName("username/nonexistrepo:latest")
	if err != nil {
		t.Fatalf("could not parse reference: %v", err)
	}
	...
	nonExistTag, err := reference.NormalizedName("username/repo1:nonexist")
	if err != nil {
		t.Fatalf("could not parse reference: %v", err)
	}
...
func TestInvalidTags(t *testing.T) {
	...
	// sha256 as repo name
	ref, err := reference.NormalizedName("sha256:abc")
	if err != nil {
		t.Fatal(err)
	}
	...
	ref, err = reference.NormalizedName("registry@sha256:367eb40fd0330a7e464777121e39d2f5b3e8e23a1e159342e53ab05c9e4d94e6")
	if err != nil {
		t.Fatal(err)
	}
	err = store.AddTag(ref, id, true)
