...
func (fms *fileMetadataStore) getLayerDirectory(l layer.ChainID) string {
	dgst := digest.Digest(l)
	return filepath.Join(fms.root, string(dgst.Algorithm()), dgst.Hex())
}

func (fms *fileMetadataStore) getLayerFilename(l layer.ChainID, filename string) string {
	return filepath.Join(fms.getLayerDirectory(l), filename)
}
...
func (fm *fileMetadataTransaction) SetParent(parent layer.ChainID) error {
	return fm.ws.WriteFile("parent", []byte(digest.Digest(parent).String()), 0644)
}

func (fm *fileMetadataTransaction) SetDiffID(diff layer.DiffID) error {
...
func (fm *fileMetadataTransaction) Commit(l layer.ChainID) error {
	finalDir := fm.store.getLayerDirectory(l)
...
func (fms *fileMetadataStore) GetSize(l layer.ChainID) (int64, error) {
	content, err := ioutil.ReadFile(fms.getLayerFilename(l, "size"))
	if err != nil {
		return 0, err
	}
...
func (fms *fileMetadataStore) GetParent(l layer.ChainID) (layer.ChainID, error) {
	content, err := ioutil.ReadFile(fms.getLayerFilename(l, "parent"))
	if err != nil {
	...
	return layer.ChainID(dgst), nil
}

func (fms *fileMetadataStore) GetDiffID(l layer.ChainID) (layer.DiffID, error) {
	content, err := ioutil.ReadFile(fms.getLayerFilename(l, "diff"))
	if err != nil {
		return "", err
	}
	...
	return layer.DiffID(dgst), nil
}

func (fms *fileMetadataStore) GetDescriptor(l layer.ChainID) (distribution.Descriptor, error) {
	content, err := ioutil.ReadFile(fms.getLayerFilename(l, "descriptor.json"))
	if err != nil {
		if os.IsNotExist(err) {
...
func (fms *fileMetadataStore) TarSplitReader(l layer.ChainID) (io.ReadCloser, error) {
	fz, err := os.Open(fms.getLayerFilename(l, "tar-split.json.gz"))
	if err != nil {
		return nil, err
	}
...
func (fms *fileMetadataStore) SetInitName(mount string, init string) error {
	if err := os.MkdirAll(fms.getMountDirectory(mount), 0755); err != nil {
		return err
	}
	return ioutil.WriteFile(fms.getMountFilename(mount, "init-name"), []byte(init), 0644)
}

func (fms *fileMetadataStore) SetMountParent(mount string, parent layer.ChainID) error {
	if err := os.MkdirAll(fms.getMountDirectory(mount), 0755); err != nil {
		return err
	}
...
func (fms *fileMetadataStore) GetMountParent(mount string) (layer.ChainID, error) {
...
func (fms *fileMetadataStore) List() ([]layer.ChainID, []string, error) {
	var ids []layer.ChainID
		...
		for _, fi := range fileInfos {
			if fi.IsDir() && fi.Name() != "mounts" {
				...
				if err := dgst.Validate(); err != nil {
					...
					ids = append(ids, layer.ChainID(dgst))
				}
			}
		}
	}
...
func (fms *fileMetadataStore) Remove(layer layer.ChainID) error {
