...
func newEndpoint(address url.URL, tlsConfig *tls.Config, userAgent string, metaHeaders http.Header) (*V1Endpoint, error) {
	endpoint := &V1Endpoint{
...
// trimmed address and an error if there is a non-V1 version.
...
func newEndpointFromStr(address string, tlsConfig *tls.Config, userAgent string, metaHeaders http.Header) (*V1Endpoint, error) {
...
func (e *V1Endpoint) Path(path string) string {
	return fmt.Sprintf("%s/v1/%s", e.URL, path)
}
